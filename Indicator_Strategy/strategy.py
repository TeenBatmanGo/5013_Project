...
my_cash_balance_lower_limit = 1500000
...
def handle_bar(timer, data, info, init_cash, transaction, detail_last_min, memory):
    ...
    if memory.spanCounter % time_span == 0 and memory.spanCounter != 0 and detail_last_min[1] > my_cash_balance_lower_limit:
        ...
        for i in range(len(asset_index1)):
            ...
            cost += num_lot * (lot_value*(1.+transaction))
        ...
        if detail_last_min[1] - cost > my_cash_balance_lower_limit:
        ...
        if detail_last_min[1] - cost > my_cash_balance_lower_limit:
            for m in range(len(asset_index2)):
                inp = generate_data_helper(memory.data_list, asset_index3[m])
                input_close = inp[:, 3]
                a = tb.RSI(input_close, timeperiod=rsi_period)

                current_average_sum = compute_average(data)
                current_average_target = current_average_sum[asset_index3[m]]
                lot_value = current_average_target * info.unit_per_lot[asset_index3[m]] * info.margin_rate[asset_index3[m]]
                num_lot = np.round(cost_rate * (weight_index3[m]) * init_cash / (lot_value * (1. + transaction)))

                # RSI
                if a[-1] < rsi_lower:
                    position_new[asset_index3[m]] += num_lot
                elif a[-1] > rsi_higher:
                    position_new[asset_index3[m]] -= num_lot
                else:
                     pass
